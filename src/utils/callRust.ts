...
import type { MemoWithResources, CreateMemoRequest, ListMemosRequest, UpdateMemoRequest } from '@/types/memo';
import type { DiaryWithMemos, CreateOrUpdateDiaryRequest, ListDiariesRequest, UpdateDiarySummaryRequest, UpdateDiaryMoodRequest } from '@/types/diary';
...
export const userCommands = {
  ...
  updateUser: (req: UpdateUserRequest) =>
    callRust<User>('update_user', { req }),

  uploadAvatar: (sourcePath: string) =>
    callRust<User>('upload_avatar', { sourcePath }),
};

export const memoCommands = {
  createMemo: (req: CreateMemoRequest) =>
    callRust<string>('create_memo', { req }),

  getMemo: (memoId: string) =>
    callRust<MemoWithResources>('get_memo', { memoId }),

  listMemos: (req: ListMemosRequest) =>
    callRust<PaginatedResponse<MemoWithResources>>('list_memos', { req }),

  getMemosByDate: (date: string) =>
    callRust<MemoWithResources[]>('get_memos_by_date', { date }),

  updateMemo: (req: UpdateMemoRequest) =>
    callRust<void>('update_memo', { req }),

  deleteMemo: (memoId: string) =>
    callRust<void>('delete_memo', { memoId }),

  archiveMemo: (memoId: string) =>
    callRust<void>('archive_memo', { memoId }),

  unarchiveMemo: (memoId: string) =>
    callRust<void>('unarchive_memo', { memoId }),
};

export const diaryCommands = {
  getDiaryByDate: (date: string) =>
    callRust<DiaryWithMemos>('get_diary_by_date', { date }),
...
export const assetCommands = {
  uploadFiles: (filePaths: string[]) =>
    callRust<UploadedResource[]>('upload_files', { filePaths }),
};
