...
- **Fix: smoother home input keyboard transitions.** The memo input now follows the native keyboard animation on the UI thread, reducing frame drops when the keyboard opens or closes.
- **Fix: filter drawer reappearing from Settings.** The search filter drawer is now mounted only while the Search tab is focused and is dismissed when leaving the screen.
...
- **Bot configuration management.** Bot editing has been fully migrated from the mobile app to the Dashboard (`/bots`). All bot fields — name, description, tags, auto-reply, avatar, and model — are now managed in one place.
- **Model override field.** Added `model` field to the bot editor, allowing per-bot model override (empty = use system default).
- **Activity refresh polish.** Clicking refresh no longer flashes skeleton placeholders — the existing activity list stays visible while new data loads.
...
- **Overflow images described as text.** When image limits are exceeded, remaining images get AI-generated text descriptions appended as context — no image vanishes silently.
...
- **High-Quality Images toggle.** Settings → General. When on, memo card grids and full-screen previews show original-resolution images immediately — no thumbnails, no "View Original" step. When off, the original blur-to-clear LQIP flow is preserved. Persisted via on-device storage.
