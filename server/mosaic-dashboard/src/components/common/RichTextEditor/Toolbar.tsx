import { cn } from '@/lib/utils'
import { Editor } from '@tiptap/react'
import {
  Bold,
  CheckSquare,
  Code,
  Code2,
  FileDown,
  Heading1,
  Heading2,
  Heading3,
  Highlighter,
  Italic,
  Link,
  List,
  ListOrdered,
  Quote,
  Redo2,
  RemoveFormatting,
  Save,
  Strikethrough,
  Underline,
  Undo2,
} from 'lucide-react'
import { useState } from 'react'
import { LinkDialog } from './LinkDialog'
import { ToolbarButton } from './ToolbarButton'

interface ToolbarProps {
  editor: Editor
  className?: string
  onToggleMarkdown?: () => void
  markdownMode?: boolean
  onOpenLinkDialog?: () => void
  onSave?: () => void
  isExpanded?: boolean
}

export function Toolbar({
  editor,
  className,
  onToggleMarkdown,
  markdownMode,
  onOpenLinkDialog,
  onSave,
  isExpanded,
}: ToolbarProps) {
  const [isLinkDialogOpen, setIsLinkDialogOpen] = useState(false)

  if (!editor || !editor.view || editor.isDestroyed) {
    return null
  }

  const formatButtons = [
    {
      icon: Bold,
      label: '粗体',
      shortcut: 'Ctrl+B',
      onClick: () => editor.chain().focus().toggleBold().run(),
      isActive: () => editor.isActive('bold'),
      canExecute: () => editor?.can()?.chain()?.focus()?.toggleBold()?.run() ?? false,
    },
    {
      icon: Italic,
      label: '斜体',
      shortcut: 'Ctrl+I',
      onClick: () => editor.chain().focus().toggleItalic().run(),
      isActive: () => editor.isActive('italic'),
      canExecute: () => editor?.can()?.chain()?.focus()?.toggleItalic()?.run() ?? false,
    },
    {
      icon: Strikethrough,
      label: '删除线',
      shortcut: 'Ctrl+Shift+X',
      onClick: () => editor.chain().focus().toggleStrike().run(),
      isActive: () => editor.isActive('strike'),
      canExecute: () => editor?.can()?.chain()?.focus()?.toggleStrike()?.run() ?? false,
    },
    {
      icon: Underline,
      label: '下划线',
      shortcut: 'Ctrl+U',
      onClick: () => editor.chain().focus().toggleUnderline().run(),
      isActive: () => editor.isActive('underline'),
      canExecute: () => editor?.can()?.chain()?.focus()?.toggleUnderline()?.run() ?? false,
    },
    {
      icon: Code,
      label: '行内代码',
      shortcut: 'Ctrl+`',
      onClick: () => editor.chain().focus().toggleCode().run(),
      isActive: () => editor.isActive('code'),
      canExecute: () => editor?.can()?.chain()?.focus()?.toggleCode()?.run() ?? false,
    },
    {
      icon: Highlighter,
      label: '高亮',
      shortcut: 'Ctrl+Shift+H',
      onClick: () => editor.chain().focus().toggleHighlight().run(),
      isActive: () => editor.isActive('highlight'),
      canExecute: () => editor?.can()?.chain()?.focus()?.toggleHighlight()?.run() ?? false,
    },
  ]

  const structureButtons = [
    {
      icon: List,
      label: '无序列表',
      shortcut: 'Ctrl+Shift+L',
      onClick: () => editor.chain().focus().toggleBulletList().run(),
      isActive: () => editor.isActive('bulletList'),
      canExecute: () => editor?.can()?.chain()?.focus()?.toggleBulletList()?.run() ?? false,
    },
    {
      icon: ListOrdered,
      label: '有序列表',
      shortcut: 'Ctrl+Shift+O',
      onClick: () => editor.chain().focus().toggleOrderedList().run(),
      isActive: () => editor.isActive('orderedList'),
      canExecute: () => editor?.can()?.chain()?.focus()?.toggleOrderedList()?.run() ?? false,
    },
    {
      icon: CheckSquare,
      label: '任务列表',
      shortcut: 'Ctrl+Shift+C',
      onClick: () => editor.chain().focus().toggleTaskList().run(),
      isActive: () => editor.isActive('taskList'),
      canExecute: () => editor?.can()?.chain()?.focus()?.toggleTaskList()?.run() ?? false,
    },
    {
      icon: Quote,
      label: '引用',
      shortcut: 'Ctrl+Shift+Q',
      onClick: () => editor.chain().focus().toggleBlockquote().run(),
      isActive: () => editor.isActive('blockquote'),
      canExecute: () => editor?.can()?.chain()?.focus()?.toggleBlockquote()?.run() ?? false,
    },
    {
      icon: Heading1,
      label: '标题 1',
      shortcut: 'Ctrl+Shift+1',
      onClick: () => editor.chain().focus().toggleHeading({ level: 1 }).run(),
      isActive: () => editor.isActive('heading', { level: 1 }),
      canExecute: () =>
        editor?.can()?.chain()?.focus()?.toggleHeading({ level: 1 })?.run() ?? false,
    },
    {
      icon: Heading2,
      label: '标题 2',
      shortcut: 'Ctrl+Shift+2',
      onClick: () => editor.chain().focus().toggleHeading({ level: 2 }).run(),
      isActive: () => editor.isActive('heading', { level: 2 }),
      canExecute: () =>
        editor?.can()?.chain()?.focus()?.toggleHeading({ level: 2 })?.run() ?? false,
    },
    {
      icon: Heading3,
      label: '标题 3',
      shortcut: 'Ctrl+Shift+3',
      onClick: () => editor.chain().focus().toggleHeading({ level: 3 }).run(),
      isActive: () => editor.isActive('heading', { level: 3 }),
      canExecute: () =>
        editor?.can()?.chain()?.focus()?.toggleHeading({ level: 3 })?.run() ?? false,
    },
  ]

  const codeButtons = [
    {
      icon: Code2,
      label: '代码块',
      shortcut: 'Ctrl+Shift+M',
      onClick: () => editor.chain().focus().toggleCodeBlock().run(),
      isActive: () => editor.isActive('codeBlock'),
      canExecute: () => editor?.can()?.chain()?.focus()?.toggleCodeBlock()?.run() ?? false,
    },
  ]

  return (
    <>
      <div className={cn('flex flex-wrap items-center gap-1 p-1 border-b', className)}>
        <div className="flex items-center gap-1">
          {formatButtons.map((btn, index) => (
            <ToolbarButton
              key={index}
              editor={editor}
              icon={btn.icon}
              label={btn.label}
              shortcut={btn.shortcut}
              onClick={btn.onClick}
              isActive={btn.isActive}
              canExecute={btn.canExecute}
            />
          ))}
        </div>

        <div className="h-4 w-px bg-border mx-1" />

        <div className="flex items-center gap-1">
          {structureButtons.map((btn, index) => (
            <ToolbarButton
              key={index}
              editor={editor}
              icon={btn.icon}
              label={btn.label}
              shortcut={btn.shortcut}
              onClick={btn.onClick}
              isActive={btn.isActive}
              canExecute={btn.canExecute}
            />
          ))}
        </div>

        <div className="h-4 w-px bg-border mx-1" />

        <div className="flex items-center gap-1">
          {codeButtons.map((btn, index) => (
            <ToolbarButton
              key={index}
              editor={editor}
              icon={btn.icon}
              label={btn.label}
              shortcut={btn.shortcut}
              onClick={btn.onClick}
              isActive={btn.isActive}
              canExecute={btn.canExecute}
            />
          ))}
        </div>

        <div className="h-4 w-px bg-border mx-1" />

        <div className="flex items-center gap-1">
          <LinkDialog
            isOpen={isLinkDialogOpen}
            onOpenChange={setIsLinkDialogOpen}
            editor={editor}
          />
          <ToolbarButton
            editor={editor}
            icon={Link}
            label="链接"
            shortcut="Ctrl+K"
            onClick={() => setIsLinkDialogOpen(true)}
            isActive={() => editor.isActive('link')}
            canExecute={() =>
              editor?.can()?.chain()?.focus().toggleLink({ href: '' })?.run() ?? false
            }
          />
        </div>

        <div className="h-4 w-px bg-border mx-1" />

        <div className="flex items-center gap-1">
          <ToolbarButton
            editor={editor}
            icon={Undo2}
            label="撤销"
            shortcut="Ctrl+Z"
            onClick={() => editor.chain().focus().undo().run()}
            isActive={() => false}
            canExecute={() => editor?.can()?.chain()?.focus().undo()?.run() ?? false}
          />
          <ToolbarButton
            editor={editor}
            icon={Redo2}
            label="重做"
            shortcut="Ctrl+Y"
            onClick={() => editor.chain().focus().redo().run()}
            isActive={() => false}
            canExecute={() => editor?.can()?.chain()?.focus().redo()?.run() ?? false}
          />
          <ToolbarButton
            editor={editor}
            icon={RemoveFormatting}
            label="清除格式"
            shortcut="Ctrl+Shift+R"
            onClick={() => editor.chain().focus().unsetAllMarks().clearNodes().run()}
            isActive={() => false}
            canExecute={() =>
              editor?.can()?.chain()?.focus().unsetAllMarks()?.clearNodes()?.run() ?? false
            }
          />
        </div>

        {onToggleMarkdown && (
          <>
            <div className="h-4 w-px bg-border mx-1" />
            <div className="flex items-center gap-1">
              <ToolbarButton
                editor={editor}
                icon={FileDown}
                label={markdownMode ? '编辑模式' : '预览模式'}
                shortcut="Ctrl+Shift+M"
                onClick={onToggleMarkdown}
                isActive={() => markdownMode ?? false}
                canExecute={() => true}
              />
            </div>
          </>
        )}

        {onSave && (
          <>
            <div className="h-4 w-px bg-border mx-1" />
            <div className="flex items-center gap-1">
              <ToolbarButton
                editor={editor}
                icon={Save}
                label="保存"
                shortcut="Ctrl+S"
                onClick={onSave}
                isActive={() => false}
                canExecute={() => true}
              />
            </div>
          </>
        )}
      </div>
    </>
  )
}
