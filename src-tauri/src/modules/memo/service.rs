...
pub async fn get_memo_by_id(
  ...
  Ok(super::models::MemoWithResources {
    memo,
    resources,
  })
}
...
pub async fn list_memos(
  ...
  let resources = if memo_ids.is_empty() {
    ...
    let placeholders = (0..memo_ids.len()).map(|_| "?").collect::<Vec<_>>().join(",");
    ...
    let mut resources_query_builder = sqlx::query_as::<_, super::models::Resource>(&resources_query);
  ...
  let items: Vec<super::models::MemoWithResources> = memos
    ...
    .map(|memo| {
      let resources = resource_map
        .remove(&memo.id)
        .unwrap_or_default();
      super::models::MemoWithResources {
        memo,
        resources,
      }
    })
